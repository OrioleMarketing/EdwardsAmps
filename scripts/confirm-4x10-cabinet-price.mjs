...
const query = `query ConfirmFourByTenCabinet {
  productByHandle(handle: "4x10-oval-open-back-speaker-cabinet") {
    id
    title
    handle
    variants(first: 10) {
...
if (!response.ok || payload.errors?.length || !payload.data?.productByHandle) {
  throw new Error(`4x10 cabinet lookup failed: ${JSON.stringify(payload.errors ?? response.status)}`);
}
