...
const query = `query ConfirmSixtyNineSeventyThreeHeadPublication {
  productByHandle(handle: "69-73-head") {
    id
    title
    handle
    status
...
if (!response.ok || payload.errors?.length || !payload.data?.productByHandle) {
  throw new Error(`69/73 Head publication lookup failed: ${JSON.stringify(payload.errors ?? response.status)}`);
}
